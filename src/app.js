...
function createServer(emailService = new EmailService()) {
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      try {
        const mailboxes = await emailService.listMailboxes(req.session.userId);
        res.json(mailboxes);
      } catch (error) {
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      try {
        const messages = await emailService.listMessages(req.session.userId, req.params.mailbox);
        res.json(messages);
      } catch (error) {
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      try {
        const message = await emailService.getMessage(req.session.userId, req.params.id);
        res.json(message);
      } catch (error) {
        res.status(404).json({ error: error.message });
      }
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      try {
        const message = await emailService.markRead(req.session.userId, req.params.id);
        res.json(message);
      } catch (error) {
        res.status(404).json({ error: error.message });
      }
